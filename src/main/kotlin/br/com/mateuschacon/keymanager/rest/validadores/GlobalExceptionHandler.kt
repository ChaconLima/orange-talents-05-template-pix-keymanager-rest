package br.com.mateuschacon.keymanager.rest.validadores
...
class GlobalExceptionHandler: ExceptionHandler<StatusRuntimeException, HttpResponse<Any>> {
    ...
    override fun handle(request: HttpRequest<*>, exception: StatusRuntimeException): HttpResponse<Any> {
        ...
        val (httpStatus, message) = when( statusCode){
            ...
            Status.INVALID_ARGUMENT.code->Pair( HttpStatus.BAD_REQUEST, "Dados de Requisição estão inválidos")
            Status.ALREADY_EXISTS.code-> Pair(HttpStatus.UNPROCESSABLE_ENTITY, statusDescription)

            else->{
                this.logger.error( "Erro inesperado '${exception.javaClass.name}' ao processar requisição", exception)
                Pair(HttpStatus.INTERNAL_SERVER_ERROR, "Não foi possivel completar a requisição devido ao erro : ${statusDescription} (${statusCode})")
            }
